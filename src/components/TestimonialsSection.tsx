...
const testimonials = [
  { name: 'Priya Sharma', role: 'Regular Customer', text: 'RivoraFresh has completely changed how I shop for groceries. The organic produce is always fresh and delivered right to my door. Love the Alphonso mangoes!', avatar: 'photo-1494790108377-be9c29b29330', rating: 5 },
  ...
  { name: 'Vikram Patel', role: 'Chef', text: 'As a professional chef, I need the freshest ingredients. RivoraFresh delivers restaurant-quality produce consistently. Highly recommended!', avatar: 'photo-1472099645785-5658abf4ff4e', rating: 5 },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" ref={ref}>
        ...
        >
          ...
          <h2 className="text-3xl md:text-5xl font-bold text-gray-900">
            What Our <span className="text-rivora-green">Customers</span> Say
          </h2>
        </motion.div>
        ...
        <div className="relative max-w-4xl mx-auto">
          ...
          >
            {testimonials.slice(current, current + 3).map((t) => (
              <motion.div
                ...
                whileHover={{ y: -6 }}
                className="p-6 rounded-3xl bg-rivora-light hover:bg-white hover:shadow-xl transition-all duration-500"
              >
                <Quote className="w-8 h-8 text-rivora-green/30 mb-4" />
                <p className="text-gray-600 leading-relaxed mb-6">{t.text}</p>
                <div className="flex items-center gap-1 mb-4">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} className={`w-4 h-4 ${i < t.rating ? 'fill-rivora-yellow text-rivora-yellow' : 'text-gray-200 fill-gray-200'}`} />
                  ))}
                </div>
          ...
          <div className="flex items-center justify-center gap-4 mt-10">
            <motion.button
              ...
              disabled={current === 0}
              className="w-10 h-10 rounded-full bg-rivora-light flex items-center justify-center text-gray-500 hover:text-rivora-green disabled:opacity-30 transition-colors"
            >
            ...
            <div className="flex gap-2">
              {[0, 3].map((i) => (
                <button
                  ...
                  className={`w-2.5 h-2.5 rounded-full transition-all ${current === i ? 'bg-rivora-green w-8' : 'bg-gray-300'}`}
                />
              ))}
            </div>
            <motion.button
